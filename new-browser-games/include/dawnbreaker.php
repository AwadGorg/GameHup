...
$sql = "INSERT INTO Dawnbreaker (cid,date,message,username)
 VALUES ('$cid','$date','$message','$username')";
 $result = $conn->query($sql);
}
}
function getcomments($conn){
  $sql = "SELECT * FROM Dawnbreaker" ;
  $result = $conn->query($sql);
...
$date = $_POST['date'];
$message = $_POST['message'];
$sql = "UPDATE Dawnbreaker SET message = '$message' WHERE cid = '$cid' ";
 $result = $conn->query($sql);
 header("Location: dawnbreaker-online.php");
 exit();
}
}
function deletecomment($conn){
  ...
  $cid = $_POST['cid'];
  $sql = "DELETE FROM Dawnbreaker WHERE cid = '$cid' ";
   $result = $conn->query($sql);
}
}
