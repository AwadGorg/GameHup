...
$sql = "INSERT INTO chronicles_of_eidola (cid,date,message,username)
 VALUES ('$cid','$date','$message','$username')";
 $result = $conn->query($sql);
}
}
function getcomments($conn){
  $sql = "SELECT * FROM chronicles_of_eidola" ;
  $result = $conn->query($sql);
...
$date = $_POST['date'];
$message = $_POST['message'];
$sql = "UPDATE chronicles_of_eidola SET message = '$message' WHERE cid = '$cid' ";
 $result = $conn->query($sql);
 header("Location: chronicles-of-eidola.php");
 exit();
}
}
function deletecomment($conn){
  ...
  $cid = $_POST['cid'];
  $sql = "DELETE FROM chronicles_of_eidola WHERE cid = '$cid' ";
   $result = $conn->query($sql);
}
}
